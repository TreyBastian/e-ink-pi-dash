...
static SM_TEXT_STYLE: MonoTextStyle<BinaryColor> = MonoTextStyle::new(&FONT_4X6, BinaryColor::On);
static MD_TEXT_STYLE: MonoTextStyle<BinaryColor> = MonoTextStyle::new(&FONT_6X10, BinaryColor::On);
static LG_TEXT_STYLE_BOLD: MonoTextStyle<BinaryColor> = MonoTextStyle::new(&FONT_9X15_BOLD, BinaryColor::On);
static STROKE_STYLE: PrimitiveStyle<BinaryColor> = PrimitiveStyle::with_stroke(BinaryColor::On, 1);
...
pub fn draw_update_text<D>(display: &mut D, next_update_time: Option<Instant>) -> Result<(), D::Error> where D: DrawTarget<Color=BinaryColor> {
    let text = match next_update_time {
        ...
        // we add +1 to the time because we don't want to show 0
...
pub fn draw_gmail_box<D>(display: &mut D) -> Result<(), D::Error> where D: DrawTarget<Color=BinaryColor> {
    ...
    Text::new("E-mail Inbox", Point::new(10, 28), LG_TEXT_STYLE_BOLD).draw(display)?;
    let mut next = 35;
    for _ in 0..13 {
